...
const DEMO_BREAKS: Record<string, string> = {
  ...
  events:      'Events silently lost when the broker is down between DB commit and publish.',
  circuit:     'A slow dependency pins every thread until the entire system queues to death.',
  vault:       'Six-month-old static passwords leak via logs, screenshots, and alumni laptops.',
  idempotency: 'Every network retry creates a duplicate charge, order, or email.',
  stampede:    'A popular cache key expires and a thousand requests stampede the database.',
